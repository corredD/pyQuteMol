...
class quaternion:
    ...
    def __mul__(self, other):
        if isQuaternion(other):
            return quaternion(numpy.dot(self.asMatrix(), other.asMatrix())[:, 0])
        else:
    ...
    def __truediv__(self, other):
        if isinstance(other, (int, float)):
            return quaternion(self.array / other)
        raise TypeError(
            f"Unsupported operand type(s) for /: 'quaternion' and '{type(other).__name__}'"
        )
    ...
    def asMatrix(self):
        ...
        @rtype: C{numpy.array}
        """
        if numpy.fabs(self.norm() - 1.0) > 1.0e-5:
...
if __name__ == "__main__":
    ...
    axis = Vector(1.0, -2.0, 1.0).normal()
    phi = 0.2
    sin_phi_2 = numpy.sin(0.5 * phi)
    cos_phi_2 = numpy.cos(0.5 * phi)
    quat = quaternion(
        cos_phi_2, sin_phi_2 * axis[0], sin_phi_2 * axis[1], sin_phi_2 * axis[2]
    )
    rot = quat.asRotation()

...
class Canvas:
    ...
    def InitRes(self):
        ...
        for i in range(NTRIES):
            ...
            if depth:
                code1 = tryme[i]
                code2 = GL.GL_DEPTH_COMPONENT
                code3 = GL.GL_UNSIGNED_INT
            else:
                code1 = GL.GL_RGBA8
                code2 = GL.GL_RGBA
                code3 = GL.GL_UNSIGNED_BYTE
            glTexImage2D(
                GL.GL_TEXTURE_2D,
                0,
                code1,
            ...
            if depth:
                code1 = GL_DEPTH_ATTACHMENT_EXT
            else:
                code1 = GL_COLOR_ATTACHMENT0_EXT
            glFramebufferTexture2DEXT(
                GL_FRAMEBUFFER_EXT,
                code1,
                GL.GL_TEXTURE_2D,
                self.textureID[self.currentRes],
                0,
            )
            if depth:
                GL.glTexParameteri(
                ...
                GL.glTexParameteri(
                    GL.GL_TEXTURE_2D, GL_DEPTH_TEXTURE_MODE, GL.GL_LUMINANCE
                )
                if hide:
                    ...
                    glRenderbufferStorageEXT(
                        GL_RENDERBUFFER_EXT, GL.GL_RGBA, screensizex, screensizey
                    )
            ...
            if use_depth:
                if Canvas.dummydepth == 666:
                    Canvas.dummydepth = glGenRenderbuffersEXT(1)
                glBindRenderbufferEXT(GL_RENDERBUFFER_EXT, Canvas.dummydepth)
                glRenderbufferStorageEXT(
                    GL_RENDERBUFFER_EXT, tryme[i], screensizex, screensizey
                )

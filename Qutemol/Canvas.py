...
class Canvas:
    ...
    def CheckFrameBuffer():
        ...
        if res == GL_FRAMEBUFFER_COMPLETE_EXT: return True
        else: return False
    ...
    def InitRes(self):
        ...
        self.frameID[self.currentRes] = glGenFramebuffersEXT(1)
        self.textureID[self.currentRes] = GL.glGenTextures(1)
        glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, self.frameID[self.currentRes])
        tryme = [GL_DEPTH_COMPONENT24, GL_DEPTH_COMPONENT16]
        ...
        for i in range(NTRIES):
            ...
            if (depth): code1 = GL_DEPTH_ATTACHMENT_EXT
            else: code1 = GL_COLOR_ATTACHMENT0_EXT
            glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, code1, GL.GL_TEXTURE_2D, self.textureID[self.currentRes], 0)
            if (depth):
                GL.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MIN_FILTER, GL.GL_NEAREST)
                GL.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_TEXTURE_MAG_FILTER, GL.GL_NEAREST)
                GL.glTexParameteri(GL.GL_TEXTURE_2D, GL_DEPTH_TEXTURE_MODE, GL.GL_LUMINANCE)
                if (hide):
                    # TODO: render to buffer NONE
                    renderID = glGenRenderbuffersEXT(1)
                    glBindRenderbufferEXT(GL_RENDERBUFFER_EXT, renderID )
                    glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL.GL_RGBA, screensizex, screensizey)
                    glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT,
                            GL_RENDERBUFFER_EXT, renderID )
            if (use_depth):
                if Canvas.dummydepth == 666: Canvas.dummydepth = glGenRenderbuffersEXT(1)
                glBindRenderbufferEXT(GL_RENDERBUFFER_EXT, Canvas.dummydepth )
                glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, tryme[i], screensizex, screensizey )
                # attach it to framebuffer
                glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT, Canvas.dummydepth)
    ...
    def SetAsOutput(self):
        if (self.onVideo): glBindFramebufferEXT(GL_FRAMEBUFFER_EXT,0)
        else:
    ...
    def CheckFrameBuffer(self):
        res = glCheckFramebufferStatusEXT(GL_FRAMEBUFFER_EXT)
        if (res == GL_FRAMEBUFFER_COMPLETE_EXT): return True
        elif (res == GL_FRAMEBUFFER_UNSUPPORTED_EXT):
            print( "Unsupported FB!\n")
            return False
        elif (res == GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT_EXT):
            print( "Incomplete: attachment !\n")
            return False
        elif (res == GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT_EXT):
            print( "Incomplete: missing attach  FB!\n")
            return False
        elif (res == GL_FRAMEBUFFER_INCOMPLETE_DUPLICATE_ATTACHMENT_EXT):
            print( "Incomplete: duplicate attach  FB!\n")
            return False
        elif (res == GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT):
            print( "Incomplete: dimensions!\n")
            return False
        elif (res == GL_FRAMEBUFFER_INCOMPLETE_FORMATS_EXT):
            print( "Incomplete: formats!\n")
            return False
        elif (res == GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER_EXT):
            print( "Incomplete: draw buffer!\n")
            return False
        elif (res == GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER_EXT):

...
class CgUtil:
    ...
    def __init__(self):
        self.P_light_base = 0.0  # from 0 (dark) to 1 (bright)
        self.P_lighting = 0.0  # from 0 (no lighting) to 1 (full lighting)
        self.P_phong = 0.0  # from 0 (no phong lighting) to 1 (full lighting)
        self.P_phong_size = 0.0  # from 0 (POW=100) to 1 (POW=1)

        self.P_col_atoms_sat = 0.0  # base color: saturation
        self.P_col_atoms_bri = 0.0  # base color: brightness

        self.P_texture = 0.0  # FOR AO! from 0 (no AO) to 1 full AO
        self.P_border_inside = 0.0  # size of antialiased borders inside, in % of atom
        self.P_border_outside = 0.0  # borders outside (pure black), full size
        self.P_depth_full = 0.0  # size of depth step for a full border
        ...
        self.P_halo_col = 0.0
        self.P_halo_str = 0.0
        self.P_halo_aware = 0.0

        self.P_fog = 0.0
        self.P_capping = False  # capping
        self.P_shadowstrenght = 0.0  # how much light

        self.P_bg_color_R = 0.0
        self.P_bg_color_G = 0.0
        self.P_bg_color_B = 0.0

        self.P_double_shadows = False
        self.P_border_fixed = False

        # questi sono settati automaticamente
        self.gap = 0.0  # gap, % of border texels to be skipped
        self.writeAlpha = False  # True during probe rendering only
        self.writeCol = False  # write a color at all?
        self.cyl_radius = 0.0

        self.doingAlphaSnapshot = (
            False  # lots of things change when doing an alpha snapshot
        )
    ...
    def _border_outside(self):
        return self.P_border_outside * 0.075

    def _border_inside(self):
        return self.P_border_inside * 0.5

    def UseHalo(self):
        return self.P_halo_size**2 > 0.0
    ...
    def SetForShadowMap(
        self, accurate
    ):  # set default params for an offscreen rendering
        self.P_light_base = 0.0
        self.P_lighting = 0.0
        self.P_phong = 0.0
    ...
    def MakeStickShaders(self):
        if self.idfStick == 666:
            self.idfStick = glGenProgramsARB(1)
        glBindProgramARB(GL_FRAGMENT_PROGRAM_ARB, self.idfStick)
        self.setStickFragmentProgram()

        if self.idvStick == 666:
            self.idvStick = glGenProgramsARB(1)
    ...
    def BindStickShaders(self):
        if not self.loadedStick:
            self.MakeStickShaders()
        glBindProgramARB(GL_FRAGMENT_PROGRAM_ARB, self.idfStick)
        glBindProgramARB(GL_VERTEX_PROGRAM_ARB, self.idvStick)
...
MOV origpos.w, 1;\n"""
        ...
        glProgramStringARB(
            GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB, len(fp), fp
        )
        if not _checkProgramError(fp):
            return False
        else:
            return True
...
MOV origpos.w, 1;\n"""
    ...
    def BindHaloShader(self, powres):  # 1^powres = size of halo texture
        ...
        if not self.loadedHalo[powres]:
            self.MakeHaloShader(powres)

        if self.idvHalo == 666:
            self.idvHalo = glGenProgramsARB(1)
        glBindProgramARB(GL_VERTEX_PROGRAM_ARB, self.idvHalo)
        if not self.loadedVertexHalo:
            self.LoadVertexHaloShader()
...
MUL tmp.z, tmp.z, tmp2.x;  # again for smoother edges\n""" % (
        ...
        if self.P_halo_str < 1.0:
            fp += "MUL tmp.z , tmp.z, %10.8f;\n" % (self.P_halo_str)
        if not self.doingAlphaSnapshot:
            ...
            if self.P_halo_col == 1.0:
                fp += "MOV result.color, tmp.z;\n"
            else:
                fp += "MUL result.color, {0,0,0,1}, tmp.z;\n"
        fp += "END\n"
...
\n"""
        ...
        if self.P_texture > 0:
            vertex_prog += "ATTRIB offset = vertex.texcoord;\n"
...
"""
        ...
        if (self.P_col_atoms_sat > 0) and (self.P_col_atoms_bri > 0):
            vertex_prog += "MOV result.color, vertex.color;\n"
        if self.P_texture > 0:
            vertex_prog += "MOV result.texcoord[2], offset;\n"
        if self.P_use_shadowmap():
            vertex_prog += " MOV result.texcoord[3], vertex.position;\n"
        vertex_prog += "\nEND\n"
        ...
        if not _checkProgramError(vertex_prog):
            return False
        else:
            return True
...
END\n"""
            ...
            if not _checkProgramError(frag_prog):
                return False
            else:
                return True
...
ATTRIB origpos = fragment.texcoord[3];  \n"""
        ...
        if moltextureCanvas.GetHardRes() == 0:
            TNORM = 0
        else:
            TNORM = 1.0 / moltextureCanvas.GetHardRes()
        frag_prog += """\n\
...
KIL tmp2.y;  \n""" % (
                -2 * self._border_outside(),
                self._border_outside() * self._border_outside(),
            )
        else:
            frag_prog += """\n\nKIL -border.x;  \n"""
...
MOV n.w, 0;                   \n\n"""
        ...
        if self.P_use_shadowmap():
            frag_prog += self.addShadowMapComputation()
...
\nEND\n"""
        ...
        glProgramStringARB(
            GL_VERTEX_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB, len(vp), vp
        )
        if not _checkProgramError(vp):
            return False
        else:
            return True
    ...
    def addDirectLightingSnippet(self):
        ...
        if not (self.P_sem_effect):
            lighting = self.P_lighting
        else:
            lighting = 1 - self.P_lighting
...
ADD t.z, -t1.z, t0.z; \n"""
        ...
        if not self.do_use_doubleshadow() and self.P_light_base > 0:
        ...
        if self.P_col_atoms_sat > 0:
            if (self.P_col_atoms_sat < 1) or (self.P_col_atoms_bri < 1):
                prog += "MAD tmp, %5.3f, basecol,%5.3f;\n" % (
                    self.P_col_atoms_sat * self.P_col_atoms_bri,
                    (1.0 - self.P_col_atoms_sat) * self.P_col_atoms_bri,
                )
                prog += "MUL res, res, tmp;\n"
            else:
                prog += "MUL res, res, basecol;\n"
        else:
            if self.P_col_atoms_bri < 1.0:
                prog += "MUL res, res, %5.3f;\n" % (self.P_col_atoms_bri)

        if self.writeCol:
            if self.P_phong > 0:
                prog += "LRP res, lighting.y, 1, res;\n"

        if self.writeAlpha:
            prog += "MOV res.w, nor.z;\n"
        # UNUSED:
        if self._border_inside() > 0:
            prog += """\n\
...
\n""" % (
                -depth_full / self._border_outside() / 20000.0
            )  # -0.001)

        if self.P_capping:
            prog += "ADD result.depth, depth.x, 0.001;\n"
        else:
            prog += "MOV result.depth, depth.x;\n"
        ...
        if (self.P_capping) and ((lighting > 0) or (self.P_phong > 0)):
            prog += """\n\
...
CMP l.x, l.x, -l.x, l.x; # DOUBLE SIDE\n\n"""
        ...
        if hardSettings.NVIDIA_PATCH:
            nvidia_patch = "MUL l.x, 0.5, param.w;          # <-- patch! REMOVE ME when N-VIDIA wakes up \n"
        else:
            nvidia_patch = ""
        ...
        print(
            (
                ...
                % (
                    ...
                    self.P_phong,
                    self.P_phong_size,
                    self.P_col_atoms_sat,
                    self.P_col_atoms_bri,
                    self.P_texture,
                    self.P_border_inside,
                    self.P_border_outside,
                    self.P_depth_full,
                    self.P_sem_effect,
                    self.P_halo_size,
                    self.P_halo_col,
                    ...
                    self.P_fog,
                    self.P_capping,
                    self.P_shadowstrenght,
                    self.P_bg_color_R,
                    self.P_bg_color_G,
                    self.P_bg_color_B,
                    self.auto_normalize,
                    self.P_double_shadows,
                    self.P_border_fixed,
                )
            )
        )

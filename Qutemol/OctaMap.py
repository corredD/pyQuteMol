...
class OctaMapSamp:
    ...
    def getDir(self, x, y):
        ...
        # create point -
        p = numpy.array((x * 2.0 / fs - 1.0, y * 2.0 / fs - 1, 0))
        ax = numpy.abs(p[0])
        ay = numpy.abs(p[1])
        az = 1
        if ax + ay > 1.0:
            p = numpy.array(
                (numpy.sign(p[0]) * (1 - ay), numpy.sign(p[1]) * (1 - ax), 0)
            )
            az = -1
        p[2] = (1 - ax - ay) * az
        # Normalize
    ...
    def TotTexSizeX(self):
        return self.size

    def TotTexSizeY(self):
        return self.size

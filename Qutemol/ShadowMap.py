...
def setMatrices(L, screensize, screensizeHard, sx, shadowmap):
    ...
    if numpy.dot(ax, ax) < 0.1:
        ax = cross(az, numpy.array([0, 1, 0], numpy.float32))
...
matFinal = numpy.zeros(
    (4, 4), numpy.float32
)  # matrix for FP computation  = matSM x (MV)^(-1)
...
class ShadowMap:
    ...
    def prepareDepthTextureForCurrentViewpoint(self):
        ...
        if use_accurate_halo:
            self.shadowSettingsAcc.BindShaders()
        else:
            self.shadowSettings.BindShaders()
...
class AOgpu2:
    ...
    def __init__(self, dir, mol, ndir, shadowmap):
        ...
        glProgramEnvParameter4fARB(
            GL_FRAGMENT_PROGRAM_ARB, 3, dir[0], dir[1], dir[2], 4.0 / ndir
        )
    ...
    @staticmethod
    def Bind():
        if not moltextureCanvas.SetAsOutput():
            raise Exception("CAnt bind moltexture canvas")
...
class OctaLevel:
    ...
    def Init(self, lev):
        ...
        if lev == 0:
            self.v[0, 0] = numpy.array([0, 0, -1])
            self.v[0, 1] = numpy.array([0, 1, 0])
            self.v[0, 2] = numpy.array([0, 0, -1])
            self.v[1, 0] = numpy.array([-1, 0, 0])
            self.v[1, 1] = numpy.array([0, 0, 1])
            self.v[1, 2] = numpy.array([1, 0, 0])
            self.v[2, 0] = numpy.array([0, 0, -1])
            self.v[2, 1] = numpy.array([0, -1, 0])
            self.v[2, 2] = numpy.array([0, 0, -1])
        else:
            tmp = OctaLevel()
            tmp.Init(lev - 1)
            ...
            for i in range(self.sz):
                for j in range(self.sz):
                    ...
                    if (i % 2) != 0 and (j % 2) == 0:
                        self.v[i, j] = (
                            tmp.v[int(i / 2) + 0, int(j / 2)]
                            + tmp.v[int(i / 2) + 1, int(j / 2)]
                        ) / 2.0
                    if (i % 2) == 0 and (j % 2) != 0:
                        self.v[i, j] = (
                            tmp.v[int(i / 2), int(j / 2) + 0]
                            + tmp.v[int(i / 2), int(j / 2) + 1]
                        ) / 2.0
                    if (i % 2) != 0 and (j % 2) != 0:
                        self.v[i, j] = (
                            tmp.v[int(i / 2) + 0, int(j / 2) + 0]
                            + tmp.v[int(i / 2) + 0, int(j / 2) + 1]
                            + tmp.v[int(i / 2) + 1, int(j / 2) + 0]
                            + tmp.v[int(i / 2) + 1, int(j / 2) + 1]
                        ) / 4.0

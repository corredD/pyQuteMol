...
def getAtomColor(atom):
    ...
    if color == 0:
        color = E2C.get(atom[:2], 0)
    if color == 0:
        color = E2C.get(atom[:1], 0)
    color_int = [ord(val) for val in struct.unpack("cccc", struct.pack("i", color))]
    return numpy.array(color_int[1:]) / 255.0
...
class Atom:
    def __init__(self, atomid, name):
        ...
        self.col = numpy.array(getAtomColor(name)) / 255.0
    ...
    def Draw(self):
        ...
        glTexCoord2f(
            self.tx / moltextureCanvas.GetHardRes(),
            self.ty / moltextureCanvas.GetHardRes(),
        )
        glNormal3f(1, 1, r)
    ...
    def FillTexture(self, texture, texsize):
        octamap.FillTexture(
            texture, texsize, self.tx, self.ty, self.col[0], self.col[1], self.col[2]
        )

    def AssignNextTextPos(self, texsize):
        self.tx = lx
        self.ty = ly
        if (lx + octamap.TotTexSizeX() > texsize) or (
            ly + octamap.TotTexSizeY() > texsize
        ):
            return False
    ...
    def DrawOnTexture(self, CSIZE, px, py, pz, r):
        ...
        glTexCoord2f(Xm, Ym)
        glVertex2f(-h + self.tx, -h + self.ty)
        glTexCoord2f(Xp, Ym)
        glVertex2f(-h + self.tx + CSIZE, -h + self.ty)
        glTexCoord2f(Xp, Yp)
        glVertex2f(-h + self.tx + CSIZE, -h + self.ty + CSIZE)
        glTexCoord2f(Xm, Yp)
        glVertex2f(-h + self.tx, -h + self.ty + CSIZE)
    ...
    def DrawHalo(self, r, px, py, pz):
        ...
        glMultiTexCoord2fARB(
            GL_TEXTURE1_ARB, r + s, (r + s) * (r + s) / (s * s + 2 * r * s)
        )
        glTexCoord2f(+1, +1)
        glVertex3f(px, py, pz)
        glTexCoord2f(-1, +1)
...
class Molecule:
    ...
    def read_next_frame(self):
        if self.istrj:
            ...
            if currframe == len(self.universe.dcd):
                currframe = 0
            ts = self.universe.dcd[
                currframe
            ]  # this looks weird, but currframe is 1-indexed

            if (
                self.averaging > 1
                and not ts.frame > len(self.universe.dcd) - self.averaging
            ):
                self.new_ts *= 0
    ...
    def Draw(self):
        ...
        for i in range(3):
            glProgramEnvParameter4fARB(
                GL_FRAGMENT_PROGRAM_ARB, i, x[i][0], x[i][1], x[i][2], 0
            )
        glProgramEnvParameter4fARB(
            GL_FRAGMENT_PROGRAM_ARB, 6, self.PredictAO(), 0, 0, 0
        )
        ...
        molGL.MolDraw(
            self.atompos,
            self.radii,
            self.textures / moltextureCanvas.GetHardRes(),
            self.colors,
            self.clipplane,
            self.excl,
            self.idx,
    ...
    def DrawShadowmap(self, invert, shadowSettings):
        ...
        glBegin(GL_QUADS)
        molGL.MolDrawShadow(
            self.atompos, self.radii, self.clipplane, self.excl, self.idx
        )
        glEnd()
    ...
    def DrawHalos(self):
        ...
        glProgramEnvParameter4fARB(
            ...
            0,
            (100.0 + cgSettings.P_halo_aware * 1300.0) / scalef / r,
            0,
            0,
            0,
        )
    ...
    def DrawOnTexture(self):
        ...
        glLoadIdentity()
        glOrtho(
            0, moltextureCanvas.GetSoftRes(), 0, moltextureCanvas.GetSoftRes(), 0, 1
        )
        ...
        glBegin(GL_QUADS)
        molGL.MolDrawOnTexture(
            self.atompos, self.radii, self.textures, hardSettings.CSIZE, self.idx
        )
        glEnd()
    ...
    def PrepareAOstep(self, nsteps, shadowmap):
        if not self.DoingAO():
            return True
        if not self.AOstarted:
            self.PrepareAOstart()
        AOgpu2.Bind()
        if ShadowMap.validView(self.DirV[self.AOdoneLvl]):
            ao = AOgpu2(self.DirV[self.AOdoneLvl], self, len(self.DirV), shadowmap)
        AOgpu2.UnBind()
        self.AOdoneLvl += 1
    ...
    def DecentAO(self):
        k = 1.0
        if self.AOdoneLvl >= len(self.DirV):
            return True
        else:
            return False  # XXX
        if self.numatoms < 10:
            return self.AOdoneLvl > 6 * k
        if self.numatoms < 100:
            return self.AOdoneLvl > 4 * k
        if self.numatoms < 1000:
            return self.AOdoneLvl > 2 * k
        if self.numatoms < 10000:
            return self.AOdoneLvl > 1 * k
        return True

    def PredictAO(self):
        # multiplicative prediction
        if self.AOstarted == False:
            return 1.0
        else:
            coeff = 0.25 + (self.AOdoneLvl - 1) / 20.0
            if coeff > 1.0:
                coeff = 1.0
...
def extractCurrentScaleFactor():
    x = glGetFloatv(GL_MODELVIEW_MATRIX)
    scalef = numpy.power(numpy.abs(numpy.linalg.det(x)), 1.0 / 3.0)
    return scalef


def extractCurrentScaleFactor_x(x):
    return numpy.power(numpy.abs(numpy.linalg.det(x)), 1.0 / 3.0)
